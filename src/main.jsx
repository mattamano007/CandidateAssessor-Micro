...
import { ArrowRight, Send, Code2, CheckCircle2, Zap, Target, Users, BarChart3, Award, MessageSquare, X } from 'lucide-react';
import { motion } from 'framer-motion';

const WAITLIST_ENDPOINT = 'https://script.google.com/macros/s/REPLACE_WITH_DEPLOY_ID/exec'; // replace with your deployed Apps Script URL

const CandidateAssessor = () => {
  ...
  const submitWaitlist = async (e) => {
    e.preventDefault();
    if (!email) return setStatus({ state: 'error', message: 'Please enter an email.' });
    ...
    try {
      ...
      if (!res.ok || data.status !== 'ok') throw new Error(data.message || 'Failed to join waitlist.');
      setStatus({ state: 'success', message: 'Added to waitlist. Check your inbox!' });
      setEmail('');
    } catch (err) {
    ...
    <div className="font-sans antialiased">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-xl border-b border-slate-200/60">
        <div className="max-w-[1280px] mx-auto px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-bold text-slate-900">CandidateAssessor</span>
          </div>
          ...
          <div className="grid grid-cols-[55%_45%] gap-12 items-center">
            ...
            <motion.div {...fadeUp}>
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 rounded-full mb-6">
                ...
                <span className="text-sm font-semibold text-blue-600">See performance before day one</span>
              </div>
            ...
            >
              ...
              <div className="relative w-[480px] bg-white rounded-[24px] p-8 shadow-[0_0_0_1px_rgba(0,0,0,0.04),0_2px_4px_rgba(0,0,0,0.04),0_8px_16px_rgba(0,0,0,0.06),0_16px_32px_rgba(0,0,0,0.08)] border border-white/60 backdrop-blur-xl">
                ...
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="text-5xl font-bold bg-gradient-to-br from-blue-600 to-pink-600 bg-clip-text text-transparent">
                    82%
                  </div>
                </div>

                {/* Progress bar */}
                <div className="relative h-[10px] bg-slate-100 rounded-full overflow-hidden mb-8">
                  <motion.div 
                ...
                <div className="flex gap-2 mb-8">
                  {['Execution', 'Communication', 'Culture Fit'].map(cat => (
                    <div key={cat} className="px-4 py-2 bg-slate-50 rounded-[10px] text-[13px] font-medium text-slate-600">
                      {cat}
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-0">
                  {[
                    { label: 'Scenario Fit', value: 'High', detail: 'Clear recommendations', icon: CheckCircle2 },
                    { label: 'Technical Depth', value: 'Advanced', detail: 'Hands-on demonstration', icon: Code2 },
                    { label: 'Team Signals', value: 'Ready', detail: 'Low onboarding lift', icon: Users }
                  ].map((item, i) => (
      ...
      <section className="relative z-10 -mt-16 mb-24">
        <div className="max-w-[1100px] mx-auto px-8">
          ...
          >
            {[
              { number: '62%', label: 'Faster shortlists', sublabel: 'After replacing live interviews' },
              { number: '4.7/5', label: 'Candidate experience', sublabel: 'During pilots' },
              { number: '200+', label: 'Teams hiring', sublabel: 'On proof, not promises' }
            ].map((stat, i) => (
          ...
          >
            ...
            <motion.div className="space-y-6" variants={staggerItem}>
              <div className="inline-block px-4 py-2 bg-red-50 rounded-full">
                <span className="text-[13px] font-semibold text-red-600">Before CandidateAssessor</span>
              </div>
              ...
              <div className="space-y-4">
                {[
                  '6-hour interview marathons',
                  'Panel members with different takes',
                  'CFO asks: "But can they actually do it?"'
                ].map((item, i) => (
            ...
            <motion.div className="space-y-6" variants={staggerItem}>
              <div className="inline-block px-4 py-2 bg-green-50 rounded-full">
                <span className="text-[13px] font-semibold text-green-600">After CandidateAssessor</span>
              </div>
              <h2 className="text-[42px] font-bold text-slate-900 leading-[1.1]">
              ...
              <div className="space-y-4">
                {[
                  'One 3-hour work sample',
                  'One scorecard everyone trusts',
              ...
              {[
                {
                  number: '01',
                  title: 'Send the task',
                  description: 'Pick a template. Set a timer. Share the link.',
                  icon: Send
                },
                {
                  number: '02',
                  title: 'They do the work',
                  description: 'Real output. Not interview theater.',
                  icon: Code2
                },
                {
                  number: '03',
                  title: 'You get proof',
                  description: 'Scorecard with evidence. Hire/hold call. Done.',
                  icon: CheckCircle2
                }
              ].map((step, i) => (
      ...
      <section id="features" className="py-32 bg-slate-50">
        <div className="max-w-[1280px] mx-auto px-8">
          ...
          >
            {[
              { icon: Zap, title: 'Performance-Based Tasks', description: 'Role-specific challenges that mirror actual work.' },
              { icon: Target, title: 'Skill-Specific Evaluations', description: 'Measure the exact competencies that matter for the role.' },
              { icon: Users, title: 'Culture Fit Scoring', description: 'Assess collaboration, communication, and working style.' },
              { icon: BarChart3, title: 'Detailed Decision Reports', description: 'Evidence-backed summaries to align hiring panels.' },
              { icon: Award, title: 'Automated Candidate Ranking', description: 'Rank candidates instantly by performance and role fit.' },
              { icon: MessageSquare, title: 'Team Collaboration', description: 'Share scorecards, comments, and consensus in one view.' }
            ].map((feature, i) => (
              <motion.div
                key={i}
                className="bg-white rounded-[18px] p-8 border border-slate-200 hover:border-blue-600 hover:shadow-[0_8px_24px_rgba(37,99,235,0.12)] hover:-translate-y-1 transition-all duration-300 group"
                variants={staggerItem}
                whileHover={{ y: -4 }}
              >
                <div className="w-14 h-14 bg-gradient-to-br from-blue-50 to-purple-50 rounded-[14px] flex items-center justify-center mb-5 group-hover:scale-110 transition-transform duration-300">
      ...
      <section className="relative py-32 overflow-hidden">
        ...
        {/* Glow orbs */}
        <div className="absolute top-0 left-1/4 w-[500px] h-[500px] bg-blue-500/30 rounded-full blur-[120px]" />
        <div className="absolute bottom-0 right-1/4 w-[500px] h-[500px] bg-purple-500/30 rounded-full blur-[120px]" />
      ...
      <footer className="bg-slate-900 py-16">
        <div className="max-w-[1280px] mx-auto px-8">
          <div className="flex items-center justify-between text-slate-400 text-sm">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-gradient-to-br from-blue-600 to-purple-600 rounded-lg" />
              <span className="font-semibold text-white">CandidateAssessor</span>
            </div>
          ...
          <div className="mt-8 text-center text-slate-500 text-sm">
            © 2025 CandidateAssessor. All rights reserved.
          </div>
        </div>
      </footer>
    </div>
...
ReactDOM.createRoot(document.getElementById('root')).render(<CandidateAssessor />);

export default CandidateAssessor;
